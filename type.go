...
func tryGetDuckdbTypeFromValue(rt reflect.Type) (C.duckdb_logical_type, error) {
	...
	case reflect.Struct:
		...
		types := (*[1 << 31]C.duckdb_logical_type)(C.malloc(mallocT(uintptr(len(fields)) * unsafe.Sizeof(C.duckdb_logical_type(nil)))))
		names := (*[1 << 31]*C.char)(C.malloc(mallocT(uintptr(len(fields)) * unsafe.Sizeof((*C.char)(nil)))))
